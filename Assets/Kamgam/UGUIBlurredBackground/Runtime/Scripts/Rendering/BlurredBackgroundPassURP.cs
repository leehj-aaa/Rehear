...
namespace Kamgam.UGUIBlurredBackground
{
    public class BlurredBackgroundPassURP : ScriptableRenderPass
    {
        ...
        public Material Material
        {
            get
            {
                if (_material == null)
                {
                    ...
                    {
                        bool hasImportedFixes = UnityEditor.SessionState.GetBool("Kamgam.UGUIBlurredBackground.HasImportedFixes", false);
                        if (!hasImportedFixes)
                        {
                            ...
                            if (shouldImport)
                            {
                                ...
                                UnityEditor.SessionState.SetBool("Kamgam.UGUIBlurredBackground.HasImportedFixes", true);
                            }
                        }
                    }
#endif
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            if (Material == null)
            {
                Debug.LogWarning("Blur material is null. Maybe the shader is missing. You can try importing it manually via Tools > UGUI Blurred Background > Debug > Import Packages");
            }
